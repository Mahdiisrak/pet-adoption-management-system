...
# Find the employee-creation section using a stable marker.
...
# First DB execute after passwordHash is the PERSON insert.
$start = $c.IndexOf("await connection.execute(", $markerPos)
if ($start -lt 0) {
  throw "Could not find PERSON insert execute block."
}

# The next DB execute marks the beginning of EMPLOYEE insert.
